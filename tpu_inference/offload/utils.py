...
def jax_copy_swap_kv_caches(
    ...
    if direction == "d2h":
        ...
        for kv_cache in src_kv_caches:
            shards = [
                jax.device_put(shard.data,
                               jax.sharding.SingleDeviceSharding(shard.device, memory_kind=dst_memory_kind))
        ...
        for layer_blocks in src_kv_caches:
            ...
            for block in layer_blocks:
                if isinstance(block, list):
                    ...
                    layer_result.append(
                        jax.device_put(block, dst_sharding))
...
def get_kv_cache_swap_fn(
    ...
    """
    _swap_fn: SwapFn = (
        pallas_swap_kv_caches if swap_op_type == "pallas"
        else jax_copy_swap_kv_caches if swap_op_type == "jax_copy"
        else jax_swap_kv_caches
    )
